...
This folder was intended to include experimental code for a slightly different approach to detecting tools. The idea is to
...
Unfortunately we didn't get super far in implementing this. All that ended up being written was a script to run Segment Anything on the dataset we used for our YOLO model, just to test the results of SAM. The results (with no prompting or tuning at all) were subpar at best. It was also much more computationally intense than running predictions on a YOLO model.
