...
class DrawerOpenState:
    ...
    tool_detection_state_history: list[tuple[datetime, set[str], str]] = field(
    ...
    def detailed_state(
        ...
        "waiting_for_initial_tool_detection", "watching_for_tool_checkin_or_checkout"
    ]:
        drawer_open_delta = datetime.now() - self.time_of_drawer_open
        is_ready_for_tool_count_changes = (
            drawer_open_delta.total_seconds()
            < MS_FROM_DRAWER_OPEN_TO_WATCHING_FOR_TOOL_CHECKIN_OR_CHECKOUT / 1000
        )
    ...
    def record_tool_detection_snapshot(self, frame_base64: str):
        ...
        self.tool_detection_state_history.append(
            (now, self.current_tool_detection_state.copy(), frame_base64)
        )

        # Clean up snapshots older than 2 seconds
        cutoff_time = now - timedelta(seconds=2)
        self.tool_detection_state_history = [
            (timestamp, state, frame)
...
class InventoryStateManager:
    ...
    tool_detection_state: NoDrawerOpenState | DrawerOpenState = NoDrawerOpenState(
    ...
    def transition_to_drawer_open(self, drawer_identifier: str):
        if isinstance(self.tool_detection_state, NoDrawerOpenState):
            ...
            print(
                f"Transitioning from drawer open to drawer open. This means that we are still waiting for the initial tool detection to complete. In ${MS_FROM_DRAWER_OPEN_TO_WATCHING_FOR_TOOL_CHECKIN_OR_CHECKOUT}ms, we'll start watching for tool checkin or checkout, so you can take or return tools."
            )
    ...
    def transition_to_no_drawer_open(self):
        ...
        tool_detection_state_to_use, frame_base64 = (
            save_state._get_tool_detection_state_2_seconds_ago()
        )

        checked_out_tools = (
            save_state.initial_tool_detection_state - tool_detection_state_to_use
        )
        returned_tools = (
    ...
    def _generate_event_log_entry(
        self,
        event_type: Literal["tool_checkout", "tool_checkin"],
        user: User,
        tool: Tool,
        event_image_base64: str,
    ):
        now = datetime.now()
        timestamp = int(now.timestamp())
        self.event_log.append(
            InventoryUpdateLogEntry(
                id=str(uuid4()),
                timestamp=timestamp,
                type=event_type,
                user=user,
                tool=tool,
                eventImageUrl=event_image_base64
                if event_image_base64
                else f"https://picsum.photos/seed/{timestamp}/500",
            )
        )
